...
description: Use the custom dual-Notion MCP server to search and fetch content across two named Notion workspaces from one Codex session.
---

# Notion Multi-Workspace

Use this plugin when the user needs access to two separate Notion workspaces in
the same session.
...
- Do not infer that a teamspace in one workspace exists in the other.
